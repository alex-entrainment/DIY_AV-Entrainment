...
def validate_path(path_str, check_exists=True, is_dir=False, is_file=False, create_dir_if_not_exist=False):
    ...
    try:
         ...
         # Use resolve() to get absolute path and check canonical form
         path = Path(path_str_cleaned).resolve()
    except Exception as e:
         return None, f"Invalid path format '{path_str}': {e}"

    if not check_exists:
         # Still return resolved path even if not checking existence
         return path, None

    if is_dir:
        if path.is_dir():
            return path, None
        elif create_dir_if_not_exist: # This option is generally not used here
             try:
...
def main():
    ...
    if config_path.exists():
        try:
            ...
            if reconfigure != 'y':
                ...
                config = configparser.ConfigParser()
                print("Reconfiguring all settings...")
        except Exception as e:
    ...
    if is_windows:
        ...
        default_serial_options = ['/dev/ttyACM0', '/dev/ttyUSB0'] # Common Pi options
    ...
    # === Default Audio Volume (Required for Controller on Both Platforms) ===
    default_volume_str = existing_controller_cfg.get('default_volume', '0.7') # Default to 70%
    while True:
        volume_str = get_input_with_default("Default Audio Volume (0.0 to 1.0)", default_volume_str)
        try:
            volume = float(volume_str)
            if 0.0 <= volume <= 1.0: # Allow 0.0 to 1.0
                config['Controller']['default_volume'] = volume_str
                break
            else:
                print("Error: Volume must be between 0.0 and 1.0.")
        except ValueError:
            print("Error: Invalid input. Please enter a number (e.g., 0.5).")
    ...
    if is_windows:
        ...
        # Set empty values for consistency if the converter script accidentally reads them on Linux (though it shouldn't run there)
