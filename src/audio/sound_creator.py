...
def noise_am(duration, sample_rate=44100, **params):
    ...
    if max_fm - min_fm > 1e-6:  # Avoid division by zero if flat
...
def isochronic_tone(duration, sample_rate=44100, **params):
    ...
    env = trapezoid_envelope_vectorized(t_in_cycle, cycle_len_array, ramp_percent, gap_percent)
...
def generate_wav(track_data, output_filename=None):
    ...
    if track_audio is None or track_audio.size == 0:
        ...
        # --- apply a fixed headroom peak‐normalization ---
    peak = np.max(np.abs(track_audio))
    if peak > 1e-9:
        gain = min(1.0, 0.9 / peak)   # scale so peak ≤ 0.9
        track_audio *= gain

    # Convert directly to 16‑bit PCM (no further normalization)
    track_int16 = np.int16(track_audio * 32767)
