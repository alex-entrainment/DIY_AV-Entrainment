...
The codebase is divided into a standalone `audio` package and a separate `visual` package. Common utilities can be placed under `common`. If you only need the synthesis tools you can work solely from the `audio` folder.
...
* **Audio:** Sequences can include audio definitions (up to 3 carriers, binaural/isochronic modes, pink noise, frequency ramps, RFM). Audio waveform files (`.wav`, `.flac`, `.mp3`) can be generated by the GUI editor using the integrated `sound_creator.py` engine. Crucially, audio playback is handled by the **host computer** running the `visual/src/controller.py` script, synchronized with the start of the LED sequence on the ESP32.
...
* **Automated C++ Generation & Upload:** A Python script (`json_to_cpp_converter.py`) automatically:
  ...
  * Updates the `visual/src/controller.py` script with the new sequence name.
  * Compiles and uploads the updated firmware to the ESP32 via PlatformIO.
* **Cross-Platform Configuration:** `visual/src/setup.py` script simplifies configuration (serial ports, paths) for use on both Windows (development) and Raspberry Pi (mobile control).
...
See the [Audio README](audio/README.md) for documentation on the audio package. This contains detailed information on the capabilities of the audio generation software created here.
...
2. Run: `python visual/src/setup.py` (or `python3 visual/src/setup.py` on Pi).
3. Follow the prompts to enter:
    * The correct **Serial Port** for the ESP32 (e.g., `COM3` on Windows, `/dev/ttyACM0` on Pi).
    * (On Windows Only) Paths to your PlatformIO project, `visual/src/controller.py`, `platformio.exe`, and the PlatformIO environment name.
...
1. **Create/Save Sequence:** Use `visual/src/sequence_editor.py` -> `my_sequence.json`. Place it (and optional audio file `my_sequence.wav/flac/mp3`) in the script directory.
2. **Convert/Upload:** Run `python json_to_cpp_converter.py` on Windows. It handles updating C++ files and uploading to ESP32.
3. **Control:** Run `python visual/src/controller.py` on Windows or Pi.
